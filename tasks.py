...
def coverage(c):
    ...
    """
    c.run("coverage run --source dj-proposals-candidates runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/dj-proposals-candidates.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ dj_proposals_candidates")
...
def lint(c):
    ...
    """
    c.run("flake8 dj-proposals-candidates tests")
...
def release(c, bumpsize=''):
    ...
    import dj_proposals_candidates
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=dj_proposals_candidates.__version__))
    c.run("git push --tags")
